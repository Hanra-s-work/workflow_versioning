...
function create_homepage_man {
    ...
    local HOMEPAGE="${M_COMMENT} Manpage for Asper Header project
${M_COMMENT} Contact: Henry Letellier ${EMAIL}.
.TH ASPER HEADER ${SC_QUOTE}Asperguide${SC_QUOTE} 6 ${SC_QUOTE}September 2025${SC_QUOTE} ${SC_QUOTE}Version 1.0${SC_QUOTE} ${SC_QUOTE}Asper Header Manual${SC_QUOTE}
.PP
.SH NAME
...
.SH SYNOPSIS
.nf
.BI Asper${SC_SPACE}Header${SC_SPACE}${SC_TAB}${SC_TAB}${SC_SPACE}${SC_SPACE}${SC_SPACE}${SC_SPACE}-${SC_SPACE}${MSI_RESET}${SC_SPACE}https://asperguide\\&.github\\&.io/asper-header/
.BI Asper${SC_SPACE}Header${SC_SPACE}documentation${SC_SPACE}-${SC_SPACE}${MSI_RESET}${SC_SPACE}https://asperguide\\&.github\\&.io/asper-header/
.fi
.SH DESCRIPTION
.nf
...
.nf
.BI ${MSI_RESET}Website:${SC_SPACE}https://asperguide\\&.github\\&.io/asper-header/
.BI ${MSI_RESET}Source${SC_SPACE}code${SC_SPACE}(Github):${SC_SPACE}https://github\\&.com/Asperguide/asper-header
.BI ${MSI_RESET}Documentation:${SC_SPACE}https://asperguide\\&.github\\&.io/asper-header/
.fi
.PP
.SH DISCLAIMER
.PP
...
function update_man_paths {
    MAN_DEST="$1"
    # Update the shell man paths to include the Asper Header folder
    echo "Updating the shell man paths to include the Asper Header folder"
    ...
    echo "Updated the shell man paths to include the Asper Header folder"
}
...
function update_man_db {
    ...
    if grep -qF "$MY_MAN_PATH" "$DESTINATION"; then
        echo "The man paths for Asper Header are correct"
        return
    fi
    echo "#" >>"$DESTINATION"
    echo "#---------------------------------------------------------" >>"$DESTINATION"
...
decho "Removed '$MAN_DEST' and '$MAN_DIR/Asper Header.3' if they existed"
...
add_required_mans "$MAN_SHORTCUT_HOME" "$MAN_DEST" "Asper Header.6"
...
echo "Installation complete. You can now use 'man Asper Header' to access the manual page."
